  ...
  return (
    <>
      ...
      <button 
        ...
        className={`lg:hidden fixed top-4 right-4 z-60 flex flex-col justify-center items-center gap-1.5 p-2 px-2.5 min-w-14 transition-opacity duration-1000 pointer-events-auto ${isVisible ? "opacity-100" : "opacity-0"}`}
        style={{ 
      ...
      >
        <span className="font-mono text-(--c-ash) leading-none" style={{ fontSize: "0.65rem", letterSpacing: "0.15em", textShadow: "1px 1px 2px rgba(0,0,0,0.9)" }}>
          {isMobileOpen ? "CLOSE" : "MENU"}
        </span>
        <div className="relative w-6 h-3">
          <span 
            className={`absolute left-0 w-full h-0.5 bg-(--c-ash) transition-all duration-300 ${
      ...
      {isMobileOpen && (
        ...
        >
          ...
          <div className="absolute top-0 right-0 bottom-0 w-[55%] bg-black/50 backdrop-blur-sm shadow-[-10px_0_30px_rgba(0,0,0,0.8)] border-l border-(--c-border)" />
        </div>
      )}
    </>
  );
